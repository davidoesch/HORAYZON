...
cdef extern from "shadow_comp.h" namespace "shapes":
    cdef cppclass CppTerrain:
        ...
        void init_from_raster(
            float*, int, int, int, int, float*, float*, int, int, char*)
...
cdef class Terrain:
    ...
    def init_from_raster(
        self, np.ndarray[np.float32_t, ndim = 1] vert_grid,
        int dem_dim_0, int dem_dim_1,
        int offset_0, int offset_1,
        np.ndarray[np.float32_t, ndim = 3] vec_tilt,
        np.ndarray[np.float32_t, ndim = 3] vec_norm,
        str geom_type="grid"):
        """Initialise Terrain class with raster/grid elevation model.
